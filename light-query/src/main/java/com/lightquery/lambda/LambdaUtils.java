...
public final class LambdaUtils {
     ...
     */
    public static <C> String getPropertyName(SFunction<C, ?> lambda) {
        return PROPERTY_CACHE.computeIfAbsent(lambda, f -> {
     ...
     */
    public static <C> Class<?> getImplClass(SFunction<C, ?> lambda) {
        return IMPL_CLASS_CACHE.computeIfAbsent(lambda.getClass(), k -> {
            String className = serialize(lambda).getImplClass().replace('/', '.');
    ...
    private static SerializedLambda serialize(SFunction<?, ?> lambda) {
        try {
